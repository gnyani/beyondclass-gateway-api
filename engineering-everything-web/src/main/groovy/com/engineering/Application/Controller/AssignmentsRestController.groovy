...
class AssignmentsRestController {
    ...
    @RequestMapping(value="/users/assignments" ,method= RequestMethod.POST)
    public  String[] retrievedefault (@RequestBody Subject subjects, HttpServletRequest request, HttpServletResponse response)
    {
        ...
        for (String fl : filelist){
            println(fl);
            links[i++] = "http://localhost:8080/users/assignments/"+fl;
        }
    ...
    @RequestMapping(value="/users/assignments/{filename.+}",produces = "application/pdf" ,method= RequestMethod.GET)
    public byte[] retrieveAssignment(@RequestParam(value = "filename", required = true, defaultValue = "OU-VASV-CSE-B-4-1-DM-gnyani007@gmail.com") Object filename){
